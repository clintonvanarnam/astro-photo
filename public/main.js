...
// Wait for window.init3DText to be defined before calling it
function waitForInit3DText(retries = 20) {
  if (typeof window.init3DText === 'function') {
    window.init3DText();
  } else if (retries > 0) {
    setTimeout(() => waitForInit3DText(retries - 1), 100);
  } else {
    console.error('init3DText not found after waiting.');
  }
}
